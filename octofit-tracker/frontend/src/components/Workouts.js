...
function Workouts() {
  ...
  useEffect(() => {
    ...
    fetch(endpoint)
      ...
      .then((data) => {
        console.log('Workouts fetch response:', data);
  ...
  const items = workouts.filter((workout) =>
      ...
      <div className="card shadow-sm">
        ...
        <div className="card-body">
          ...
          {loading ? (
            ...
            <div className="alert alert-warning mb-0">No workouts found.</div>
          ) : (
                ...
                <tbody>
                  {items.map((workout, index) => (
                    <tr key={workout.id ?? index}>
                      ...
                      <td>
                        <pre className="mb-0 small text-break">{JSON.stringify(workout, null, 2)}</pre>
                      </td>
                    </tr>
                  ))}
                </tbody>
